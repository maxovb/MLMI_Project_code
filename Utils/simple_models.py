...
def SVM_classifier(X_train, y_train, X_validation, y_validation, X_test, y_test, cs=[1e-2,1,1e2,1e4,1e6,1e8,1e10], max_iter = 100):
    max_validation_accuracy_SVM = 0
    optimal_c = 1
    for c in cs:
        model = SVC(C=c, max_iter=max_iter).fit(X_train, y_train)
        validation_accuracy = model.score(X_validation, y_validation)

        if validation_accuracy > max_validation_accuracy_SVM:
            optimal_c = c
            max_validation_accuracy_SVM = validation_accuracy
    model = SVC(C=optimal_c, max_iter=max_iter).fit(X_train, y_train)
    return model.score(X_test, y_test), optimal_c