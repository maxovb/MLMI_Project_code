...
def train_joint(train_data,model,epochs, model_save_dir, train_joint_loss_dir_txt, train_unsup_loss_dir_txt, train_accuracy_dir_txt, validation_data = None, validation_joint_loss_dir_txt = "", validation_unsup_loss_dir_txt = "", validation_accuracy_dir_txt = "", semantics=False, convolutional=False, min_context_points = 2, report_freq = 100, learning_rate=1e-3, weight_decay=1e-5, save_freq = 10, n_best_checkpoint = None, epoch_start = 0, device=torch.device('cpu'), l_sup=10, l_unsup=1):
        ...
        for batch_idx, (data, target) in enumerate(iterator):
            ...
            if convolutional:
                ...
                x_context, y_context, x_target, y_target = image_processor(data, num_context_points,
                                                                           ...
                                                                           semantic_blocks=semantic_blocks,
                                                                           device=device)
                train_joint_loss, train_sup_loss, train_unsup_loss, accuracy, total = model.joint_train_step(x_context, y_context, target, y_target, opt, l_sup=l_sup, l_unsup=l_unsup)
